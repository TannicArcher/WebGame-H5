...
<div>
  ...
  <div><span>账号: </span><input type='text' value='' id='uid' placeholder='请输入账号'><input type='button' value='加入VIP' id='addvipbtn'></div>
  <div><span>充值: </span><input type='number' id='chargenum' placeholder='请输入充值数量'><input type='button' value='充值' id='chargebtn'></div>
  <div><span>开通: </span><td><select class="form-control selectpicker" id="chargenum2" name="num"><option value="440000">至尊特权</option><option value="140000">月卡</option></select>
  ...
  $('#chargebtn').click(function(){
    ...
    $('#chargebtn2').click(function(){
	  ...
	  $.ajax({
		  url:'gmquery.php',
		  type:'post',
		  'data':{type:'charge',checknum:checknum,uid:uid,num:chargenum2,qu:qu},
          'cache':false,
          'dataType':'json',
		  success:function(data){
			  console.log('data',data);
