...
<div class="gameDiv">
    <div id="gbox">
        ...
        <div style="" class="tc mainbg">
            ...
            $isnew = "hot";
            if ($lastserver) {
                echo "<div class=\"enter ma login_game\" server=" . $lastserver[0]['name'] . " style=\"margin-top: 33px;\" data-href=\"" . $url . $ver . "app.php?user=$_SESSION[username]&spverify=$_SESSION[token]&srvid=".$lastserver[0]['id']."&srvaddr=".$lastserver[0]['ip']."&srvport=".$lastserver[0]['port']."\"></div>";
            }
            ?>
        </div>
        <div id="server_all" class="pa choosebg hide">
            ...
            <div id="scrollwrap" class="scrollwrap tc bd">
                ...
                <?php
                if ($lastserver) {
                        echo "<div class=\"items login_game items_last_login_list\" server=" . $lastserver[0]['name'] . " data-href=\"" . $url . $ver . "app.php?user=$_SESSION[username]&spverify=$_SESSION[token]&srvid=".$lastserver[0]['id']."&srvaddr=".$lastserver[0]['ip']."&srvport=".$lastserver[0]['port']."\"><div class=\"item0 ma\" >" . $lastserver[0]['name'] . "</div><div class=\"" . $isnew . "\"></div></div>";
                }
                //获取所有服务器
                ...
                for ($i = count($server);$i > 0;$i--) {
                    echo "<div class=\"items login_game hide " . $tag . "\" server=" . $server[$i - 1]['name'] . " data-href=\"" . $url . $ver . "app.php?user=$_SESSION[username]&spverify=$_SESSION[token]&srvid=".$server[$i - 1]['id']."&srvaddr=".$server[$i - 1]['ip']."&srvport=".$server[$i - 1]['port']."\"><div class=\"item0 ma\" >" . $server[$i - 1]['name'] . "</div><div class=\"" . $isnew . "\"></div></div>";
                }
                ?>
            </div>
            <!-- 显示左边选项卡 -->
